...
Both **Check for Updates** inside Broadcast and the Start Menu's manual updater use that same verified installer path. Running a downloaded installer manually uses the same application-only update behavior. The default application location is `%LOCALAPPDATA%\Programs\Tempest Broadcast System`, but the installer allows any empty custom folder or existing Tempest installation and records that choice for future updates and removal. It blocks drive roots, the settings folder, and unrelated non-empty folders. It never writes to or removes `%APPDATA%\tempest-broadcast-system`, where preferences, profiles, scenes, plugin configuration, generated overlays, and the managed Asset Vault are stored. Release builds fail if their installer payload contains a configuration folder or a portable-mode marker.
...
3. Run the installer. It defaults to `%LOCALAPPDATA%\Programs\Tempest Broadcast System`, allows any empty custom folder or existing Tempest installation, adds Start Menu shortcuts, and optionally adds a desktop shortcut.
