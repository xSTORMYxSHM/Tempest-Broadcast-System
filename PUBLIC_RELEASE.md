...
The Asset Library accepts ordinary third-party stream assets; Tempest-specific markup is not required. Indexed folders can contain standard static and animated images (including PNG, JPEG, GIF, TGA, and WebP), common FFmpeg-supported video and audio files (including MP4, MOV, WebM, MKV, MP3, WAV, FLAC, and OGG), local HTML/HTM browser elements, SVG graphics, and text or JSON support assets. Images, videos, and browser elements can be added directly to the active scene with their matching OBS source type.
...
Both **Check for Updates** inside Broadcast and the Start Menu's manual updater use that same verified installer path. Running a downloaded installer manually uses the same application-only update behavior. The default application location is `%LOCALAPPDATA%\Programs\Tempest Broadcast System`, but the installer allows any empty writable custom folder or existing writable Tempest installation and records that choice for future updates and removal. Before extraction, it creates and write-tests a new selected folder; repairs and updates also write-test critical application, theme, and plugin subdirectories before overwriting files. Protected locations such as Program Files may require a different installer privilege level. It blocks drive roots, the settings folder, and unrelated non-empty folders. A failed fresh install removes only the known partial application payload, while a failed update never removes the existing installation. Broadcast resolves packaged themes, locale files, libobs shaders, and plugin modules from the executable location when a launcher supplies an unexpected working directory. It canonicalizes module search paths so an executable-relative path and an equivalent working-directory fallback cannot load the same plugin twice. Installer-created application and updater shortcuts also use the executable directory as their working folder. It never writes to or removes `%APPDATA%\tempest-broadcast-system`, where preferences, profiles, scenes, plugin configuration, generated overlays, and the managed Asset Vault are stored. Release builds fail if their installer payload contains a configuration folder or a portable-mode marker.
...
3. Run the installer. It defaults to `%LOCALAPPDATA%\Programs\Tempest Broadcast System`, allows any empty writable custom folder or existing writable Tempest installation, adds Start Menu shortcuts, and optionally adds a desktop shortcut.
