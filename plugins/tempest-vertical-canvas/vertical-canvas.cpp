...
OBS_MODULE_USE_DEFAULT_LOCALE("tempest-vertical-canvas", "en-US")
...
static obs_data_t *load_canvas_config(char *managed_path)
{
	obs_data_t *config = obs_data_create_from_json_file_safe(managed_path, "bak");
#ifdef TEMPEST_MANAGED_VERTICAL_CANVAS
	if (!config) {
		const QFileInfo managed_file(QString::fromUtf8(managed_path));
		QDir plugin_config_directory = managed_file.dir();
		if (plugin_config_directory.cdUp()) {
			const QString legacy_path = plugin_config_directory.filePath(QStringLiteral("vertical-canvas/config.json"));
			const QByteArray legacy_path_utf8 = QDir::cleanPath(legacy_path).toUtf8();
			config = obs_data_create_from_json_file_safe(legacy_path_utf8.constData(), "bak");
			if (config) {
				ensure_directory(managed_path);
				if (obs_data_save_json_safe(config, managed_path, "tmp", "bak")) {
					blog(LOG_INFO, "[Tempest Portrait] Adopted existing Aitum Vertical configuration");
				} else {
					blog(LOG_WARNING,
					     "[Tempest Portrait] Loaded legacy Aitum configuration but could not persist the managed copy");
				}
			}
		}
	}
...
bool obs_module_load(void)
{
	...
	blog(LOG_INFO, "[Tempest Portrait] loaded Aitum Vertical foundation %s", PROJECT_VERSION);
	...
	proc_handler_add(ph, "void tempest_vertical_get_video(in int width, in int height, out ptr video)", get_video, nullptr);
	proc_handler_add(ph, "void tempest_vertical_get_stream_settings(in int width, in int height, out ptr outputs)",
			 get_stream_settings, nullptr);
	proc_handler_add(ph, "void tempest_vertical_set_stream_settings(in int width, in int height, in ptr outputs)",
			 set_stream_settings, nullptr);
	proc_handler_add(ph, "void tempest_vertical_get_stream_output(in int width, in int height, in string name, out ptr output)",
			 get_stream_output, nullptr);
	proc_handler_add(ph, "void tempest_vertical_start_stream_output(in int width, in int height, in string name)",
			 start_stream_output, nullptr);
	proc_handler_add(ph, "void tempest_vertical_stop_stream_output(in int width, in int height, in string name)",
			 stop_stream_output, nullptr);
	proc_handler_add(ph, "void tempest_vertical_add_chapter(in int width, in int height, in string chapter_name)", add_chapter,
			 nullptr);
...
void obs_module_post_load(void)
{
	...
	obs_data_t *config = load_canvas_config(path);
	bfree(path);
	if (!config) {
		config = obs_data_create();
...
#ifndef TEMPEST_MANAGED_VERTICAL_CANVAS
...
#else
	blog(LOG_INFO, "[Tempest Portrait] Standalone update checks disabled; updates are managed by Broadcast");
#endif
}
