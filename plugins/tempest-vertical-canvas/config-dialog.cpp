...
void OBSBasicSettings::LoadSettings()
{
	...
	bool enable = !obs_frontend_streaming_active() && !obs_frontend_recording_active() &&
		      !obs_frontend_replay_buffer_active() && !obs_frontend_virtualcam_active() &&
		      !obs_output_active(canvasDock->recordOutput) && !obs_output_active(canvasDock->virtualCamOutput);
...
void OBSBasicSettings::SaveSettings()
{
	...
	bool resolutionChangeAllowed = !obs_frontend_streaming_active() && !obs_frontend_recording_active() &&
				       !obs_frontend_replay_buffer_active() && !obs_frontend_virtualcam_active() &&
				       !obs_output_active(canvasDock->recordOutput) &&
				       !obs_output_active(canvasDock->virtualCamOutput);
	for (const auto &output : canvasDock->streamOutputs) {
		if (obs_output_active(output.output)) {
	...
	if (sscanf(res.toUtf8().constData(), "%dx%d", &width, &height) == 2 && width > 0 && height > 0 &&
		...
		if (!resolutionChangeAllowed) {
			blog(LOG_WARNING, "[Tempest Portrait] Ignored a resolution change while an output is active");
